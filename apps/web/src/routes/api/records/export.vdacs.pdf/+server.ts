...
export const GET: RequestHandler = async (event) => {
  ...
  const signatureText = `Generated by CropCard v${APP_VERSION} on ${generatedAt.toISOString()} · exported by ${identityLabel(user)}`;
      ...
      if (currentPage === 1) {
        return {
          stack: [
            ...
            {
              text: `VDACS audit pack · ${generatedAt.toISOString().slice(0, 10)} · ${identityLabel(user)} · ${filterLine}`,
              style: 'farmSub'
            }
          ],
          margin: [40, 24, 40, 0]
