...
Status: **Sprints 30A, 30B, 30C, 30D, 30E and 30F implemented** (2026-09-26). Owner: Shawn. 30G (cards everywhere) is not built yet. Panel decisions are recorded in [Decisions (2026-09-26)](#decisions-2026-09-26).
...
**Follow-up (30H, 2026-09-26).** Two of the deferred items are done. Emergency contacts: the owner adds up to five (name, role, phone) on `/settings/farm`, with a one-tap "Add Poison Control (1-800-222-1222)" for the national US Poison Help line. They are stored as the per-Owner setting `farm_emergency_contacts` (`lib/farm/emergencyContacts.ts` + `.server.ts`), carried in the card snapshot as `emergencyContacts`, and printed at the top of the Farm Map Card on screen, in print and in the offline deck. OpenAPI: every Phase 30 endpoint is in `static/openapi.json` (geocode, hints, the card snapshot, `/api/garden/**`, `/api/fields` and `/api/blocks` with `?kind`, details and layout fields, and the client record id header on the six record endpoints). Request bodies are generated from the same Zod schemas the routes validate with (`lib/farm/apiSchemas.ts`, `lib/garden/api.ts`, `lib/hints.ts`, re-exported from each route as `_requestSchema`); `gen:openapi` now runs under `tsx`, and a unit test fails if a route and the published schema drift apart. `PATCH /api/crops/:id` (including `set-placement`) is still undocumented.
...
- **Map lines and points** (above), with migration `0054_map_features`.
- **Emergency contacts and OpenAPI** (see 30F). The integration also published `/api/map-features` and `/api/map-features/{id}`, with their request schemas moved to `lib/farm/apiSchemas.ts` and covered by the same drift test.

Still open: dragging a bed preset onto the canvas, `PATCH /api/crops/:id` in OpenAPI, moving the six record endpoints' request schemas out of their routes, telling the two "Fence" buttons in the Add drawer apart, and a real touch e2e for drag.
