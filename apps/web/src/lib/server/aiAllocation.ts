...
export function allocateDeterministic(
  ...
  return engineFallback(input, matrix, reason, detail);
}
  ...
  return {
    ...
    rationale:
      reason === 'no-api-key'
        ...
        : reason === 'over-cap'
          ? 'Plan generated by the deterministic engine (monthly AI cap reached — raise the cap on /settings/ai to restore AI refinement).'
          : reason === 'quota-exceeded'
            ? 'Plan generated by the deterministic engine (today’s AI quota reached — retry tomorrow or raise the quota on /settings/ai).'
            : reason === 'ai-unavailable'
              ? `Plan generated by the deterministic engine (${detail ?? 'Claude is unavailable right now.'})`
