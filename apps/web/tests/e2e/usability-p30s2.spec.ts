...
test.describe('persona usability fixes', () => {
  ...
  test('"Plan a crop here" on an empty Area plants the whole Area as one bed', async ({
    page
  }) => {
    await provisionEmptyFarm(page);
    ...
    await expect(where.getByRole('heading', { name: 'Where in Hayfield will this grow?' })).toBeVisible();
