...
- **Hardiness zone v1.1 and year-crossing frost seasons** (see Decisions, Hardiness zone). The designer's new server recipe, linked-sowing clash check and Area Card bed map pass the season's spring frost too, so a year-crossing season bounds occupancy the same way everywhere.
...
**Implemented (v1.1, 2026-09-26).** NOAA's normals archive has no extreme-minimum column, so `scripts/build-frost-normals.mjs` now also reads GHCN-Daily 1991-2020 from the same AWS Open Data mirror (`noaa-ghcn-pds`, `csv.gz/by_year`, 30 files with pinned SHA-256s; the mirror rewrites them daily, so the pins describe the 2026-09-26 bytes and a rebuild re-pins on purpose). For each station a year counts when December to February (June to August south of the equator) each have at least 25 QC-clean `TMIN` days, and `extremeMinF` is the mean of the counted years, written when at least 15 of 30 count (5,093 of 7,094 stations). `lib/climate/zone.ts` holds `zoneFromExtremeMin` (1a at -60 °F, 5 °F halves, clamped to 1a..13b) and the nearest-station lookup, which skips stations without the value. /settings/farm shows "Zone 7a (approx., from Washington DC Dulles AP, VA)" with a Weather service provenance chip and a "Change zone" picker that saves `farm.hardiness_zone` + `farm.hardiness_zone_provenance = manual` (blank goes back to the estimate; the estimate itself is never stored). The Farm Map Card shows the same value as a Zone fact, carried on the snapshot as `hardinessZone`. Both hide the chip when there is no station within 50 miles and no override.
