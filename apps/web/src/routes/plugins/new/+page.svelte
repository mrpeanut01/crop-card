<script lang="ts">
  ...
  function buildCropPayload() {
    ...
    return {
      ...
      version: '1.0.0',
      cropFamily,
      ...(cropDtmMin && cropDtmMax
        ? { daysToMaturity: { min: cropDtmMin, max: cropDtmMax } }
        : {}),
...
{#if !data.canEdit}
  <section class="card role-locked">
    ...
    <p>
      Authoring plugins changes the safety knowledge base for the whole farm.
      Sign in as Owner to use this wizard. Helpers can browse the existing
      catalog at <a href="/plugins">/plugins</a>.
    </p>
    <a class="primary" href="/signin">Sign in</a>
  </section>
{:else}
<p class="lede">
  Fill in the guided form. The kernel validates against the schema +
  bypass-attempt matrix at registration. Plugins claiming herbicide safety
  on a crop family the chemistry kills are rejected.
</p>

<section class="card">
  <h2>Type</h2>
  <div class="modes">
    <button class="mode" class:active={mode === 'crop'} onclick={() => (mode = 'crop')}>Crop variety</button>
    <button class="mode" class:active={mode === 'herbicide'} onclick={() => (mode = 'herbicide')}>Herbicide</button>
  </div>
</section>
...
{#if mode === 'crop'}
  <section class="card">
    ...
    <label class="full">
      ...
      <textarea rows="4" bind:value={cropIndicators}
        placeholder="e.g.
Husks fully dry and papery
Black layer at kernel tip"></textarea>
    </label>
    <label class="full">
      Notes
  ...
  <section class="card">
    ...
    <div class="grid">
      ...
      <label>
        ...
        <input type="text" bind:value={hSafeForCropIds} placeholder="e.g. corn-bantam-sweet, corn-bloody-butcher" />
      </label>
...
<style>
  ...
  .mode.active { background: #1f5e3a; color: white; border-color: #1f5e3a; }
  .grid {
    display: grid;
  ...
  label.full { display: block; margin-top: 0.75rem; }
  ...
  textarea { font-family: monospace; min-height: auto; }
  pre {
    background: #f5f5f5;
    padding: 0.75rem;
    border-radius: 4px;
