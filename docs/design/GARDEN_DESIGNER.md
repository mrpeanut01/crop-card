...
- **Size.** From the Area's typed Size (`width_ft` × `length_ft`) when set. Otherwise from the drawn polygon's bounding box in feet, with the polygon outline drawn faintly inside it for context. Otherwise 20×30 ft with a banner: "This garden has no Size yet. Set one on the Area Card so beds fit." (`canvasFromArea`, `source: 'dimensions' | 'polygon-bbox' | 'default'`).
...
**Drag to place (tablet, desktop).** Drag a crop row onto a bed with Pointer Events (not HTML5 drag and drop, so touch and mouse behave alike). A ghost footprint follows the pointer, snapped to 6 in, tinted red where it would share space in time.
...
- The page's universal loader fetches `GET /api/garden/areas/[id]/design`. On a network failure it builds the same `GardenDesign` from the Owner's offline snapshot (`designFromSnapshot`), which gains each planting's `footprint`, `spacingPattern`, `groupId` and `groupSystemKind`.
...
The Area Card for a garden or greenhouse gains a **Garden bed map** section in its print variant (§7 of the Phase 30 doc): a black-and-white SVG of the beds to scale with names, what is in each bed on the print date (patterns, not color alone), a scale bar, the north arrow when known, and a legend listing each bed's plantings with dates and counts. **Print** in the designer opens the Area Card print view with `?on=` set to the scrubber date. It works offline from the snapshot.
...
Through `components/ui/Hint.svelte` and `lib/client/hints.ts`, at most one per view, never over a safety STOP.
...
Queued offline edits, moving linked sowings across beds, free rotation angles, non-rectangular beds, editing landmarks in the designer, greenhouse season-extension offsets on frost dates, and a USDA zone anywhere in the designer.
