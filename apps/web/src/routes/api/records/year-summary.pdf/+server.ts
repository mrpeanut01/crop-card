...
export const GET: RequestHandler = async (event) => {
  ...
  const signatureText = `Generated by CropCard v${APP_VERSION} on ${now.toISOString()} · exported by ${identityLabel(user)}`;
      ...
      if (currentPage === 1) {
        return {
          stack: [
            ...
            {
              text: `${now.toISOString().slice(0, 10)} · ${identityLabel(user)}`,
              style: 'farmSub'
            }
          ],
          margin: [40, 20, 40, 0]
