...
**Follow-ups done (2026-09-26, designer sprint 3).** A crop row can be dragged from the crop panel onto a bed with Pointer Events: a mouse drags the row itself, touch drags only from the row's 48px handle so the list still scrolls, a ghost shows the spot `placeCrop` will pick (rust when the bed has no room on that date) and Escape puts it back. **Print** now refreshes the card snapshot when there is signal and opens the Area Card at `/cards/area/<key>?on=<scrubbed date>&print=1`, which draws the bed map for that date and opens the print dialog once. **Add N plantings** from a bed recipe is one `POST /api/garden/beds/[blockId]/recipe` (owner only): the server recomputes the recipe from the bed as stored with the same `applyRecipe`, saves the kept steps as `planned` plantings tagged `plugin` in one transaction, and refuses the whole commit with `STALE` when a kept step no longer fits. A linked succession sowing that is not yet in the ground can move to another bed (drag or **Move**, or **Move to bed** in the bed sheet and List view); it keeps its group link, and the server refuses a spot another planting holds while it grows there (`OVERLAP`), a spot past the bed edge (`OUTSIDE_AREA`) and a sowing already in the ground (`IN_GROUND`). The page now loads through a universal loader over `GET /api/garden/areas/[id]/design`, so a client-side visit with no signal renders read-only from the card snapshot (`designerDataFromSnapshot`) and reloads itself when the connection comes back. Dragging a bed preset onto the canvas is still tap to place. The adjacency gap stays at 4 ft, so the high-tunnel scenario's beds 4.5 ft apart get no keep-apart hint; widen it or move the beds if that hint matters. When a footprint is narrower than its row spacing the count uses one row rather than zero.
...
**Follow-up (30H, 2026-09-26).** Two of the deferred items are done. Emergency contacts: the owner adds up to five (name, role, phone) on `/settings/farm`, with a one-tap "Add Poison Control (1-800-222-1222)" for the national US Poison Help line. They are stored as the per-Owner setting `farm_emergency_contacts` (`lib/farm/emergencyContacts.ts` + `.server.ts`), carried in the card snapshot as `emergencyContacts`, and printed at the top of the Farm Map Card on screen, in print and in the offline deck. OpenAPI: every Phase 30 endpoint is in `static/openapi.json` (geocode, hints, the card snapshot, `/api/garden/**`, `/api/fields` and `/api/blocks` with `?kind`, details and layout fields, and the client record id header on the six record endpoints). Request bodies are generated from the same Zod schemas the routes validate with (`lib/farm/apiSchemas.ts`, `lib/garden/api.ts`, `lib/hints.ts`, re-exported from each route as `_requestSchema`); `gen:openapi` now runs under `tsx`, and a unit test fails if a route and the published schema drift apart. `PATCH /api/crops/:id` is published for `set-placement` only; its other actions are still undocumented.
...
- The /today and /plan Linux visual baselines need re-capturing.

**Map lines and points implemented (30H, 2026-09-26).** A tenant-scoped `map_features` table (`fence | gate | water_source | hydrant | irrigation_line | path`, GeoJSON LineString or Point, optional Area link, `details_json`) with a repo in `lib/db/mapFeatures.ts` and `/api/map-features` CRUD (owner-only writes, helpers read, another Owner's Area refused). Pure rules live in `lib/farm/mapFeatures.ts`: which kinds are lines or points, geometry checks, walked length in feet, and water source details (`well | municipal | pond | rain`, optional flow in gallons per minute). The Add drawer has a "Lines & points" group drawn with Geoman polyline or marker, Filter toggles each kind, and the list under the map edits names, Areas and water details. The Farm Map Card lists them by kind with lengths, adds them to the legend in plain color words, and draws them in its figure on the same plane as the Areas; on a farm sketched only by size they are counted, never placed. The table arrives in migration `0056_map_features`, after 30G's `0054_planting_journal` and the `0055_db_maintenance` indexes. In the Add drawer the new line kind and the shade source under "Shade & structures" are both called "Fence"; telling them apart better is a possible follow-up.

**30H follow-ups shipped (2026-09-26).** One integration branch (`p30h`) merged five follow-up tracks, and it was then merged with 30G:

- **Offline sync claim fencing.** `claimClientRecord` returns a token (the claim's `updated_at`), and `completeClientRecord` / `releaseClientRecord` only act on a pending receipt whose token still matches, so a slow request whose stale claim was taken over can no longer complete or release the new holder's claim. No migration. After #448 made each record endpoint write its event and receipt in one `writeRecord` transaction, the fenced completion moved inside that transaction (scout and journal saves now go through `writeRecord` too): when the token no longer matches, `markClientRecordSaved` throws `ClientRecordClaimLost`, the record write rolls back, and the wrapper answers `200 duplicate` if the new holder already saved or `503` while it is still saving, so a stale takeover can no longer save the record twice. Retention pruning of abandoned `pending` receipts (older than a day) fences out their old holder the same way.
- **Garden designer extras** (see 30E): drag to place, Print through the Area Card, server-side bed recipes, cross-bed linked sowings and the offline client-side designer load.
- **Year-crossing frost seasons** (see Decisions, Hardiness zone; the zone itself is main's implementation, which replaced this branch's copy in the merge). The designer's new server recipe, linked-sowing clash check and Area Card bed map pass the season's spring frost too, so a year-crossing season bounds occupancy the same way everywhere.
- **Map lines and points** (above), with migration `0056_map_features`.
- **Emergency contacts and OpenAPI** (see 30F). The integration also published `/api/map-features` and `/api/map-features/{id}`, with their request schemas moved to `lib/farm/apiSchemas.ts` and covered by the same drift test. Merging with 30G added its endpoints too: `POST /api/tasks/close`, `GET /api/records/{kind}/{id}/card`, the planting journal (`/api/plantings/{id}/journal`, the entry delete and photo, and the `/api/journal/record` replay) and `POST /api/plantings/{id}/photo-help`. Their request schemas moved into `lib/tasks/apiSchemas.ts` and `lib/journal/apiSchemas.ts` so the generator can publish them, and the drift test covers them.
- **Merge fix.** A scrubbed date's spoken summary waits half a second, and it used to overwrite an announcement made in that time, such as "5 sowings added." Any later announcement now cancels the pending summary (`announceDateSoon` in `designerState.svelte.ts`).

Still open: dragging a bed preset onto the canvas (it is still tap to place), the other `PATCH /api/crops/:id` actions in OpenAPI (only `set-placement` is published), moving the six record endpoints' request schemas out of their routes, and telling the two "Fence" buttons in the Add drawer apart. Touch drag is covered in e2e through Chromium touch events (`garden-designer-extras.spec.ts`), though not yet on a real device.
...
In v1.1 the frost build adds an `extremeMinF` column (the 1991-2020 mean annual extreme minimum) to the same station table. A pure `zoneFromExtremeMin` in `lib/climate/zone.ts` then derives an approximate zone, shown read-only on /settings/farm and the Farm Map sheet as "Zone 7a (approx., from Dulles Intl) · data" and never labelled as the USDA map. The owner can override it with `manual` provenance. Perennial and orchard work is its first consumer, and the zone never gates anything. Shipped 2026-09-26: `extremeMinF` comes from NOAA's Global Summary of the Month (the 1991-2020 mean of each year's lowest monthly EMNT, 3,500 U.S. stations with at least 20 qualifying winters; method and pinned hashes in `apps/web/src/lib/climate/data/SOURCE.md`). The owner's override is stored in `farm.hardiness_zone` with `farm.hardiness_zone_provenance = manual`; the estimate is recomputed from the location and never stored. A location with no qualifying station within 50 miles shows no zone (there is no fallback zone). Widened 2026-09-26: the farm's elevation now comes from USGS 3DEP (EPQS, public domain, cached per location), and when it is known no station more than 1,000 ft above or below the farm is used (about 3.6 °F by the standard lapse rate, under the 5 °F half-zone). If nothing qualifies within 50 miles, a second pass takes the nearest station within 100 miles at a similar elevation, shown as "Zone 6b (approx.) · nearest station 71 mi, similar elevation" with `data` provenance naming the station, and on the Farm Map Card as "6b (approx., station 71 mi)". With the elevation unknown, only the 50-mile pass runs. Still display-only, and the owner's `manual` zone still wins.

**One implementation (2026-09-26).** Phase 30H and main built this zone independently. The two were merged into the version described in the paragraph above: the GSOM-based `extremeMinF` in the frost table, `lib/climate/zone.ts` (`zoneFromExtremeMin`, `parseZone`, `lookupZone` with the 50-mile pass and the 100-mile similar-elevation pass, `farmZoneFrom` and the label helpers), `lib/climate/zoneSettings.server.ts` (`farmZone()` for loaders and `saveZoneForm()` for the owner's typed zone) and the `ZoneChip` on /settings/farm, where the owner types a zone like 7a or leaves the field blank to use the estimate. The Farm Map Card reads the same `FarmZone` from the snapshot's `zone` field. The 30H branch's GHCN-Daily build, its `zone.server.ts` and its `HardinessZoneChip` were dropped in the merge. The year-crossing frost season work from 30H (`lib/schedule/frostSeason.ts`) is unaffected.

**Year-crossing frost seasons.** About 140 warm stations have a 32 °F frost season of a few weeks around the new year, so the last spring date falls in late December or the first fall date in early January (`crossesYear`). `lib/schedule/frostSeason.ts` places each date in its calendar year: a spring date on or after the fall date in the second half of the year belongs to the year before, and otherwise the fall date belongs to the year after. `frostDatesForYear`, the designer's snapshot path (`seasonFrostMs`), `frostInYearOf` (given the season's spring frost), `scrubRange`, `scheduleCandidacy`, the planting-window check and its AI prompt all use it, so a Gulf-coast season runs Jan 31 to the next Jan 6. Onboarding no longer asks to confirm those dates; it shows a short note instead. Confirmation stays for the fallback and frost-free cases.
