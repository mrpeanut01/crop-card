<script lang="ts">
  ...
  let cellFilter = $state<'16' | '24' | 'Either' | '20' | '22' | '30' | '32' | '38' | '60'>('Either');
  ...
  function aiSeedDims(): { L: number; D: number; T: number } | null {
    const v = seedMeta?.seedDimensionsMm as
      | { L?: unknown; D?: unknown; T?: unknown }
      | undefined;
  ...
  let seedL = $state<number | undefined>(
    untrack(() => pickDisplay('displayL', 'L') ?? qpNum('l'))
  );
  let seedD = $state<number | undefined>(
    untrack(() => pickDisplay('displayD', 'D') ?? qpNum('d'))
  );
  let seedT = $state<number | undefined>(
    untrack(() => pickDisplay('displayT', 'T') ?? qpNum('t'))
  );
  ...
  const cellRec = $derived(seedType === 'Corn' ? cellCountRecommendation(targetInRowSpacing, targetRowSpacing) : null);
  ...
  const seedL64 = $derived(seedL === undefined ? undefined : dimUnit === 'mm' ? mmToInternal(seedL) : seedL);
  const seedD64 = $derived(seedD === undefined ? undefined : dimUnit === 'mm' ? mmToInternal(seedD) : seedD);
  const seedT64 = $derived(seedT === undefined ? undefined : dimUnit === 'mm' ? mmToInternal(seedT) : seedT);
  ...
  const KNOWN_COLOR = new Set([
    'red', 'blue', 'green', 'yellow', 'orange', 'pink', 'gold', 'silver', 'gray', 'grey', 'white',
    'maroon', 'olive', 'turquoise', 'tan', 'ivory', 'violet', 'coral', 'brown'
  ]);
  const COLOR_MAP: Record<string, string> = {
    'Dk. Blue': '#1a3a7a', 'Lt. Blue': '#7ec0ee', 'Med. Blue': '#3b7dd8',
    'Dk. Green': '#1f5e3a', 'Lt. Green': '#9fd99f', 'Med. Green': '#3fa75f',
    'Yel. Green': '#b8d63e', 'Lt. Gold': '#e3c97a', 'Lt. Yellow': '#fff5a8',
    'Med. Violet': '#9a5acb', 'Red-Orange': '#e34a1d', 'Orange-Red': '#e34a1d',
    'Avocado': '#6c8a2e', 'Rust': '#b7410e'
  };
...
<header class="head">
  ...
  {#if ctx}
    <p class="subtitle">Pre-filled for <strong>{ctx.displayName}</strong>. Save will write back to this seed record.</p>
  {:else}
    <p class="subtitle">Match a Lincoln Ag plate (John Deere "B" / IHC "C") to your seed by type, shape, cell count, and (optionally) dimensions. Save the result to any seed lot when you're done.</p>
  {/if}
</header>
...
<section class="card form-panel" aria-labelledby="filters-h">
  ...
  <fieldset>
    <legend>
      ...
      {#if showCellRec && cellRec && cellFilter === String(cellRec.cells) && plantsPerAcre !== null}
        <span class="rec-inline" title={cellRec.note}>· suggested ({plantsPerAcre.toLocaleString()} plants/acre)</span>
      {/if}
    </legend>
    <div class="radios">
    ...
    {#if showCellRec}
      <details class="density-disclosure">
        ...
        <p class="hint">For corn, a 24-cell plate plants 1.5× as many seeds/acre as a 16-cell at the same sprocket. Plants/acre is computed from the spacings below; the threshold (≤22k → 16-cell, ≥22k → 24-cell) is a heuristic — it picks the cell-count family, not the sprocket itself.</p>
        <div class="row2">
          <label><span>In-row spacing (in)</span><input type="number" min="0.5" step="0.5" bind:value={targetInRowSpacing} placeholder="e.g. 7.5" /></label>
          <label><span>Row spacing (in)</span><input type="number" min="6" step="1" bind:value={targetRowSpacing} placeholder="30" /></label>
        </div>
        {#if cellRec && plantsPerAcre !== null}
          <p class="rec-line rec-{cellRec.band}">
            <strong>{plantsPerAcre.toLocaleString()} plants/acre → suggests {cellRec.cells}-cell.</strong>
  ...
  <fieldset class="dims">
    <legend>
      ...
      <button type="button" class="help" aria-label="Dimensions help" title="L = Length, D = Depth, T = Thickness. Toggle between mm and 64ths of an inch.">?</button>
    </legend>
    <div class="unit-toggle" role="group" aria-label="Dimension unit">
      <button type="button" class="unit-btn" class:active={dimUnit === 'mm'} onclick={() => toggleUnit('mm')}>mm</button>
      <button type="button" class="unit-btn" class:active={dimUnit === '64ths'} onclick={() => toggleUnit('64ths')}>64ths in.</button>
    </div>
    <p class="hint">Format: L-D-T (e.g., {dimUnit === 'mm' ? '12-9-5 mm' : '30-23-13 (64ths)'}). Leave blank to see all plates for selected type.</p>
    <div class="row3">
      <label><span>L (Length, {dimUnit})</span><input type="number" min="0" step={dimUnit === 'mm' ? '0.1' : '0.5'} bind:value={seedL} /></label>
      <label><span>D (Depth, {dimUnit})</span><input type="number" min="0" step={dimUnit === 'mm' ? '0.1' : '0.5'} bind:value={seedD} /></label>
      <label><span>T (Thickness, {dimUnit})</span><input type="number" min="0" step={dimUnit === 'mm' ? '0.1' : '0.5'} bind:value={seedT} /></label>
    </div>
    <label class="block">
      <span>Tolerance: ±{tolerance} {dimUnit} per dimension</span>
      <input type="range" min="0" max={dimUnit === 'mm' ? 3 : 5} step={dimUnit === 'mm' ? 0.5 : 1} bind:value={tolerance} />
    </label>
  </fieldset>
...
<section class="card results-panel" aria-labelledby="results-h">
  <div class="results-head">
    ...
    <button type="button" class="secondary print-btn" onclick={printResults} disabled={!searched || results.length === 0}>Print results</button>
  </div>
  ...
  {:else}
    {#if seedType === 'Sugar Beet'}
      <p class="warn-banner" role="alert">⚠️ Sugar Beet plates require a Sugar Beet bottom — incompatible with standard corn planters.</p>
    {/if}
    <p class="count" aria-live="polite">
      {results.length} {results.length === 1 ? 'plate' : 'plates'} found
      ...
      <ul class="cards">
        {#each results as p (p.plateNumber)}
          <li class="plate-card">
            ...
            <dl>
              <div><dt>Dimensions</dt><dd>{p.dimensions} <small>(L-D-T, 64ths in)</small></dd></div>
              {#if p.shape}<div><dt>Shape</dt><dd><span class="badge shape-{p.shape.toLowerCase()}">{p.shape}</span></dd></div>{/if}
              <div><dt>Cells</dt><dd>{p.cells}</dd></div>
              <div><dt>Series</dt><dd>{p.series === 'B' ? 'John Deere (B)' : 'IHC (C)'}</dd></div>
              <div><dt>Seed type</dt><dd>{p.seedType}</dd></div>
              {#if p.seedType === 'Corn' && p.gradeSize}<div><dt>Grade</dt><dd><span class="badge grade">{p.gradeSize}</span></dd></div>{/if}
              {#if p.seedType === 'Sorghum' && p.notes}<div><dt>Seeds/lb</dt><dd>{p.notes}</dd></div>{/if}
              {#if p.seedType === 'Soybean' && p.notes}<div><dt>Notes</dt><dd>{p.notes}</dd></div>{/if}
              {#if 'delta' in p && p.delta !== undefined}<div><dt>Match score</dt><dd><strong>Δ = {p.delta}</strong></dd></div>{/if}
            </dl>
            ...
            {#if data.canEdit && p.seedType !== 'Sugar Beet' && data.seedItems.length > 0}
              <form method="POST" action="?/saveToStock" class="save-form">
                ...
                {#if seedL !== undefined}<input type="hidden" name="seedLDisplay" value={seedL} />{/if}
                {#if seedD !== undefined}<input type="hidden" name="seedDDisplay" value={seedD} />{/if}
                {#if seedT !== undefined}<input type="hidden" name="seedTDisplay" value={seedT} />{/if}
                <input type="hidden" name="toleranceDisplay" value={tolerance} />
                {#if targetInRowSpacing !== undefined}<input type="hidden" name="inRowInches" value={targetInRowSpacing} />{/if}
                <input type="hidden" name="rowInches" value={targetRowSpacing} />
                {#if plantsPerAcre !== null}<input type="hidden" name="plantsPerAcre" value={plantsPerAcre} />{/if}
                <button type="submit" class="primary save" disabled={!saveTargetId}>Save to seed record</button>
              </form>
...
<style>
  ...
  .card h2 {
    margin: 0 0 0.75rem; font-size: 1rem; color: #1f5e3a;
    text-transform: uppercase; letter-spacing: 0.5px;
  }
  fieldset {
    border: 1px solid #d0d7d0; border-radius: 6px;
    padding: 0.5rem 0.75rem; margin: 0 0 0.75rem;
  }
  legend {
    font-size: 0.8rem; color: #1f5e3a; font-weight: 700;
    text-transform: uppercase; padding: 0 0.25rem;
  }
  legend .opt { color: #777; font-weight: 400; text-transform: none; }
  .radios { display: flex; flex-wrap: wrap; gap: 0.75rem 1rem; }
  .radios label {
  ...
  label.block > span { font-size: 0.85rem; color: #444; }
  ...
  .dims .row3, .row2 { display: grid; gap: 0.5rem; }
  .dims .row3 { grid-template-columns: repeat(3, 1fr); }
  .row2 { grid-template-columns: repeat(2, 1fr); }
  .dims .row3 label, .row2 label {
    display: flex; flex-direction: column; font-size: 0.85rem; gap: 0.25rem;
  }
  .unit-toggle {
    display: inline-flex; border: 1px solid #d0d7d0;
    border-radius: 4px; overflow: hidden; margin-bottom: 0.4rem;
  }
  .unit-btn {
    background: white; border: none; padding: 0.4rem 0.75rem;
    font-size: 0.85rem; cursor: pointer; min-height: 36px;
    color: #444; border-right: 1px solid #d0d7d0;
  }
  .unit-btn:last-child { border-right: none; }
  .unit-btn.active { background: #1f5e3a; color: white; font-weight: 600; }
  .hint { color: #555; font-size: 0.85rem; margin: 0 0 0.5rem; }
  .help {
    border: 1px solid #1f5e3a; background: white; color: #1f5e3a;
    width: 1.4rem; height: 1.4rem; border-radius: 50%;
    font-weight: 700; cursor: help; margin-left: 0.25rem; line-height: 1;
  }
  .primary {
    background: #1f5e3a; color: white; border: none; border-radius: 6px;
    padding: 0.75rem 1.25rem; font-weight: 600; cursor: pointer; min-height: 48px;
  }
  .primary:disabled { background: #999; cursor: not-allowed; }
  .secondary {
    background: white; color: #1f5e3a; border: 2px solid #1f5e3a;
    border-radius: 6px; padding: 0.5rem 1rem; font-weight: 600; cursor: pointer; min-height: 40px;
  }
  .secondary:disabled { color: #999; border-color: #999; cursor: not-allowed; }
  .rec-inline {
    font-weight: 400; font-size: 0.75rem; color: #555;
    text-transform: none; letter-spacing: 0; margin-left: 0.4rem;
  }
  .density-disclosure {
    margin-top: 0.5rem; border-top: 1px dashed #d0d7d0; padding-top: 0.5rem;
  }
  .density-disclosure summary {
    cursor: pointer; font-size: 0.85rem; color: #1f5e3a; user-select: none;
  }
  .density-disclosure summary:hover { text-decoration: underline; }
  .density-disclosure .hint { margin: 0.5rem 0; }
  .rec-line {
    margin: 0.5rem 0 0; padding: 0.5rem 0.75rem; border-radius: 4px;
    background: #f8fbf9; border-left: 3px solid #1f5e3a; font-size: 0.9rem;
  }
  .rec-line.rec-low { border-left-color: #b35900; }
  .rec-line.rec-mid { border-left-color: #6b7280; }
  .rec-line.rec-high { border-left-color: #1f5e3a; }
  .rec-line strong { display: block; color: #1f5e3a; margin-bottom: 0.1rem; }
  .rec-line span { display: block; color: #555; font-size: 0.85rem; }
  .link-btn {
    background: none; border: none; color: #1f5e3a;
  ...
  .count { color: #444; font-size: 0.9rem; margin: 0 0 0.75rem; }
  .empty { color: #555; font-style: italic; }
  .warn-banner {
    background: #fff8ec; border: 2px solid #b35900; color: #6b3a00;
    padding: 0.6rem 0.9rem; border-radius: 6px; font-weight: 600;
  }
  .cards {
  ...
  .plate-card {
    background: #f8fbf9; border: 1px solid #d0d7d0;
    border-left: 4px solid #1f5e3a; border-radius: 6px; padding: 0.75rem 0.9rem;
  }
  .plate-card header {
    display: flex; align-items: center; justify-content: space-between;
    gap: 0.5rem; margin-bottom: 0.5rem;
  }
  .plate-num { font-size: 1.4rem; font-weight: 800; font-family: monospace; color: #1f5e3a; }
  .color-badge {
    display: inline-flex; align-items: center; gap: 0.35rem;
    background: white; border: 1px solid #d0d7d0; border-radius: 999px;
    padding: 0.15rem 0.5rem 0.15rem 0.15rem; font-size: 0.8rem; color: #333;
  }
  .swatch {
  ...
  .plate-card dl {
    ...
    gap: 0.25rem 0.75rem; margin: 0 0 0.6rem; font-size: 0.9rem;
  }
  .plate-card dl > div { display: contents; }
  .plate-card dt { color: #666; }
  .plate-card dd { margin: 0; }
  .plate-card dd small { color: #888; }
  .badge {
    display: inline-block; background: #e7f1ea; color: #1f5e3a;
    padding: 0.05rem 0.45rem; border-radius: 3px; font-size: 0.75rem;
    font-weight: 700; text-transform: uppercase;
  }
  .badge.shape-flat { background: #f0e9ff; color: #4a2d8a; }
  .badge.shape-round { background: #fef0e0; color: #8a4a14; }
  .badge.grade { background: #fffae5; color: #6b5500; }
  .save-form { margin: 0; }
  .save-target {
    display: flex; flex-direction: column; gap: 0.25rem;
    font-size: 0.8rem; color: #555; margin-bottom: 0.4rem;
  }
  .save-target select { font-size: 0.9rem; min-height: 40px; padding: 0.4rem; }
  .save { width: 100%; }
  .save-empty { font-size: 0.85rem; }
  .foot { text-align: center; color: #777; font-size: 0.85rem; padding: 1rem 0 2rem; }
  @media print {
    .head .back, .form-panel, .print-btn, .save-form, .foot { display: none !important; }
