...
{:else if w.response}
  {#if w.response.meta.fallback}
    <div class="aw-banner warn" role="alert" aria-live="assertive">
      {w.response.meta.fallback === 'no-api-key'
        ...
        : w.response.meta.fallback === 'over-cap'
          ? "This month's AI help is used up, so the deterministic engine made this plan. It works the same way; AI help resets on the 1st."
          : w.response.meta.fallback === 'quota-exceeded'
            ? "Today's AI planning limit is used up, so the deterministic engine made this plan. It resets at midnight UTC."
    ...
    {#if w.response.meta.fallback === 'over-cap' || w.response.meta.fallback === 'quota-exceeded'}
      <AiUsageChip />
    {/if}
  {/if}
