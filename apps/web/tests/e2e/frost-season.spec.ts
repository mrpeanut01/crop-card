...
test.describe('year-crossing frost seasons', () => {
  ...
  test('settings farm fits a 375px phone', async ({ page }) => {
    ...
    await expect(chip.getByTestId('zone-value')).toBeVisible();
    const chipBox = (await chip.boundingBox())!;
    const provBox = (await chip.locator('[data-provenance]').first().boundingBox())!;
    ...
    expect((await chip.getByTestId('zone-input').boundingBox())!.height).toBeGreaterThanOrEqual(48);
  });
});
