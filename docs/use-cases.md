...
- **Scope (Invariant 6):** the plugin library on disk is shared by every farm, so an Owner's changes are farm-scoped. Upload, edit (`/plugins/new?prefill=…`) and stock "save to catalog" store the validated payload in that Owner's `plugin_overrides` ([`pluginOverrides.ts`](../apps/web/src/lib/db/pluginOverrides.ts)). It replaces the shared plugin for that farm only (list chip "this farm's copy"). **Retire / Unretire** on `/plugins/[pluginId]` hides or restores the plugin for that farm only ("retired on this farm"). `getRegistry()` applies the active Owner's overlay; farm copies go through the same schema + bypass validation as shared uploads, and event replay resolves them by hash. Changing the shared library is **superadmin-only** (interactive session, never Bearer): `?scope=global` on `/api/plugins/upload`, `/retire` and `/unretire`, plus `/uninstall`, `/rollback` and `/api/plugins/rescan`. Owners don't see those controls; superadmins get "Retire for all farms", "Uninstall…", version rollback, and a "Publish to the shared library" checkbox on `/plugins/new`. Uninstall still refuses while any farm's event rows reference the plugin.
...
- **Offline read cache (NFR-02):** the production service worker (registered from `+layout.svelte`; dev mode skips it) serves previously-visited pages, SvelteKit `__data.json`, `/api/plugins` and `/api/sprayers` NetworkFirst from per-Owner runtime caches. Entries are keyed by the active Owner (`?__cc_owner=<id>`, logic in [`lib/client/swTenantKey.ts`](../apps/web/src/lib/client/swTenantKey.ts)) and only written when the server's `x-cropcard-owner` response header matches, so switching farms keeps each farm's offline cache and never serves Farm A's data under Farm B. The page announces the active Owner to the SW on every layout mount and after a switch; when the SW does not know the Owner (or a response reveals a different one) it goes network-only. Sign-out wipes every tenant cache.
- **App updates (NFR-02):** the SW uses Workbox's prompt mode, so a new version installs in the background and waits. When a waiting worker is detected (on load, on `updatefound`, or on the re-check each time the tab becomes visible) a non-blocking "New version available" toast appears (polite live region, 48px **Reload** / **Later** buttons). The app never reloads on its own — a form may hold unsaved field data — only **Reload** posts `SKIP_WAITING` and reloads once the new worker takes control (logic in [`lib/client/swUpdate.ts`](../apps/web/src/lib/client/swUpdate.ts)).
...
- **Persona:** All
- **Status:** Implemented. Landing page [`/`](../apps/web/src/routes/+page.svelte) hosts the sign-in form (`/signin` redirects there). Two server-side modes selected by `AUTH_MODE`:
  - `direct` (default; dev / demo / e2e) — email entry or a one-tap demo role mints the session immediately.
  - `magic-link` (production) — the email form sends a single-use sign-in link instead; the direct email and demo form actions return 403 server-side (not just hidden). Any unrecognised `AUTH_MODE` value fails closed to `magic-link`.
- **Trigger:** App open with no valid HMAC cookie session.
- **Primary path (magic-link):**
  1. User enters email → `POST /api/auth/magic-link` (or the landing `?/magic` action). Every well-formed request gets the same generic 200 ("If that address can sign in, a link is on its way") whether or not the account exists or the request was throttled — no user enumeration.
  2. Server mints 32 random bytes (base64url), stores only the SHA-256 hash in `login_tokens` (identity-level, not tenant-scoped; migration 0045) with a 15-minute expiry, and emails the link via `dispatchEmail` (stdout in dev, Postmark in prod). Rate limits: 5 links / email / 15 min and 20 / client IP / 15 min (DB-counted; IP stored as a hash). The client IP is whatever adapter-node's `getClientAddress()` resolves; behind Azure Container Apps ingress set `ADDRESS_HEADER=X-Forwarded-For` + `XFF_DEPTH=1` so it is the caller, not the proxy. Only a public unicast address gets its own 20-request bucket: a missing, private, loopback or unparseable address (the proxy's own address when the header is not configured) goes into one shared 200 / 15 min bucket, so a misconfiguration cannot lock every user out, and the server logs a one-time warning in production. The per-email limit applies either way.
  3. User opens `/auth/verify?token=…` → a confirm screen ("Signing in as …") that does NOT consume the token, so mail scanners that prefetch links can't burn it.
  4. "Continue" POSTs → token is atomically consumed (single use, bound to its email) → the same `loginByEmail` logic as direct sign-in runs → HMAC cookie issued → redirect by `LoginResult`: `/today` (one assignment), `/owner-picker` (several), `/onboarding` (new user — the `users` row is created only now, after the email is proven), `/admin/owners` (superadmin with no assignments).
  5. Invalid / expired / used link → friendly page naming the reason + "Send a new link" form.
- **Invites (UC-21):** `/?invite=<token>` threads the invite into the magic link (`&invite=` query param, format-checked, never stored); after verification the user lands on `/invite/<token>` to accept. Works identically in both modes.
- **Unaffected:** Phase-24 Bearer tokens (`cck_…`) are resolved in `hooks.server.ts` before any cookie lookup in both modes; the HMAC session payload (`activeOwnerId` / `activeRole` / `isSuperadmin` / `impersonating`) is unchanged.
- **Config:** `AUTH_MODE`, `ORIGIN` (required for magic-link in production — links are built from it, never from the request Host header), `EMAIL_TRANSPORT` + `POSTMARK_TOKEN`. `@auth/sveltekit` was evaluated and removed: its email provider needs an Auth.js DB adapter and its own session, which would duplicate the HMAC session layer.
- **Success:** Session cookie set; nav shows email + role badge.
- **Tests:** `lib/server/magicLink.test.ts` (issue / hash-only storage / peek / consume / expiry / single use / rate limits / no enumeration / ORIGIN), `tests/integration/magicLinkLogin.test.ts` (LoginResult routing, mode gate, Bearer unaffected), e2e `tests/e2e/auth-magic-link.spec.ts` (second preview server on `E2E_PORT+1` in magic-link mode; links captured from the test-only `/_dev/outbox`, which 404s unless `EMAIL_TRANSPORT=memory` and `E2E_OUTBOX=1`/`NODE_ENV=test`).
...
- **Persona:** P5
- **Status:** Implemented (Phase 18f, reworked as two screens in Phase 30B). [`/onboarding`](../apps/web/src/routes/onboarding/+page.svelte) asks two questions and lands the owner on `/today`, where the Getting Started card takes over.
- **Trigger:** A new user signs in for the first time. `hooks.server.ts` finds no `helper_assignments` row and sends them to `/onboarding`. Helpers who arrive through `/invite/[token]` skip onboarding entirely.
- **Primary path:**
  1. **Screen 1, "Tell us about your farm".** The farm name is prefilled as "{First name}'s Farm" when a first name can be read from the email. The location comes from an address search (US Census geocoder via `/api/geocode`, optional and silent when it finds nothing), **Use my location** (GPS), or a pin dropped on a satellite preview; typed latitude and longitude are an advanced option.
  2. As soon as a location is set, the last and first frost dates (32 °F, 50% by default, with a cautious 90% toggle) and the hard-frost dates (24 °F) fill in from the nearest NOAA 1991-2020 station within 50 miles, tagged `data` with the station name and distance. Any date can be edited inline, which tags it `manual`. With no station in range the Loudoun averages show as `fallback`, and the owner has to tick "These dates are fine for now" before continuing. The same confirmation is required for a frost-free station and for a frost season that crosses the new year, so a blank date that would silently mean Loudoun is never saved.
  3. The server creates the `owners` row, `helper_assignments(role='owner')` and `owner_subscriptions(planCode='free', status='trial')`, saves `farm_lat_lon`, the four frost dates and `frost_provenance`, and sets `onboarding_status='in-progress'`.
  4. **Screen 2, "What are you growing on?".** Four large cards, pick any: a garden, fields, hay or pasture, a greenhouse or high tunnel. Each choice creates one undrawn starter Area of the right kind (Kitchen Garden, Home Field, Hayfield, High Tunnel), and the answer is saved as `farm_profile` (`garden`, `farm` or `mixed`). "Not sure yet" skips it with no Areas.
  5. `onboarding_status` becomes `complete` (or `later` on skip) and the owner lands on `/today` with the **Getting Started** card at the top. Until screen 2 is answered, `/today` sends the owner back to it.
- **Getting Started card:** one row per item, each opening the real page: farm location, first area on the map, first crop, a garden bed (garden profiles), equipment (fields or hay), sprayer calibration (once a sprayer exists), inviting a helper (optional), the planning assistant (optional, and everything works without it), and saving cards for offline. Predicates are pure in [`lib/onboarding/gettingStarted.ts`](../apps/web/src/lib/onboarding/gettingStarted.ts). The card shrinks to a slim "Setup N of M" strip once the required items are done, disappears at 100% or on Dismiss (`getting_started_dismissed_at`), and comes back from Settings → **Re-show setup checklist**. Owners only.
- **First-use hints:** one-sentence pointers anchored to a control, shown once per person across devices (`user_hints` via `/api/me/hints`, queued locally when offline), never more than one per view and never over a safety STOP. `plan_first_crop` points at **Add planting** on `/plan`.
- **Legacy links:** the old `?step=location|fields|implements|season|plan` bookmarks 308-redirect to `/today` for any existing farm.
- **Implementation locus:** [`/onboarding`](../apps/web/src/routes/onboarding/), [`lib/onboarding/`](../apps/web/src/lib/onboarding/), [`lib/climate/frostSettings.ts`](../apps/web/src/lib/climate/frostSettings.ts), [`FrostPanel.svelte`](../apps/web/src/lib/components/onboarding/FrostPanel.svelte) (shared with `/settings/farm`), [`GettingStartedCard.svelte`](../apps/web/src/lib/components/today/GettingStartedCard.svelte), [`Hint.svelte`](../apps/web/src/lib/components/ui/Hint.svelte) + [`lib/client/hints.ts`](../apps/web/src/lib/client/hints.ts).
- **Success criteria:** A new user reaches `/today` in two screens and under a minute with GPS, with no drawing, no implements, no Season Setup and no AI key.
...
- **Success:** On-hand totals reflect every event; expiring lots and below-reorder-threshold SKUs surface on `/today` and in the top-bar **Alerts** bell (a popover listing the same alerts /today computes: decon due, winterization/recalibration, low stock, lots expiring within 30 days, offline records waiting to sync; each links to its surface, and the bell carries an active-count badge). The top bar has no Search button: there is no global search surface.
...
- **#298 — scan hardening (SSRF + Invariant 7):** `/api/scan-url` fetches the operator's URL through [`safeFetch`](../apps/web/src/lib/server/safeFetch.ts): http(s) only, no userinfo, every DNS answer must be public unicast (loopback / RFC-1918 / link-local incl. `169.254.169.254` / CGNAT / ULA / multicast / IPv4-mapped forms are rejected), the socket is pinned to the validated IP (defeats DNS rebinding), redirects are followed manually (max 5) and re-validated per hop, 12 s total timeout, 2 MB body cap, HTML only. A blocked URL or redirect returns 400; an unreachable page 502; a page with no product signal 422. The Claude calls behind `/api/scan-label`, `/api/scan-url` and the barcode enrichment in `/api/scan-barcode` all run through `aiTry()` + `aiGuard` ([`scanAi.ts`](../apps/web/src/lib/server/scanAi.ts)) with their own daily quotas (`scan-label` 40, `scan-url` 20, `scan-barcode` 40) and a 30 s timeout (45 s for URL, which includes the page fetch). Every non-AI outcome is the deterministic "not found — enter it manually" result `{found:false, source:'none', provenance:'fallback', fallbackReason, message}`: no key → 503 with the `No Anthropic API key configured` message the add-flow recovery CTAs match on (scan-url checks the key before fetching anything); daily quota → 429; monthly cap → 402; Anthropic overloaded / transient error → 503 `retryable`; timeout → 504 `retryable`. Barcode lookups degrade to a 200 not-found (OpenFoodFacts still works without a key). Each real Claude call writes an `ai_call_log` row with token usage + provenance; a call that finishes after the timeout is still metered as a `timeout` fallback row.
...
- **#255 — catalog-pick default unit.** Input plugins may carry `defaultUnit` + `formulation` (see [plugin-spec §5.5a](plugin-spec.md)). Picking a local library match in the Search method pre-fills the add form's default unit from the plugin (authored, or derived from its formulation / rate unit — e.g. a WDG product prefills `lb` instead of `fl-oz`); when the plugin gives no evidence the form keeps its per-type default and the operator picks. A missing unit never raises a validation error.
- **Phase 26 (Track A) — camera-first label scan + batch unboxing flow** _(implemented; trackers [#248](https://github.com/mrpeanut01/crop-card/issues/248) + [#249](https://github.com/mrpeanut01/crop-card/issues/249))_. The Scan-label method on `/inventory/[type]/add` (the old `/stock/add` 308-redirects there) opens a live `<video>` camera feed by default ([LabelCapture.svelte](../apps/web/src/lib/components/LabelCapture.svelte), `getUserMedia({ facingMode: 'environment' })`). File upload is the **fallback** ("Upload from gallery", or the only path when getUserMedia is unavailable) and accepts multiple files (`<input multiple>`, no `capture` attribute so mobile pickers allow multi-select). Picking one photo keeps the single-shot path. Picking two or more starts the **batch unboxing queue** ([labelBatch.svelte.ts](../apps/web/src/lib/stock/labelBatch.svelte.ts) + [LabelBatchQueue.svelte](../apps/web/src/lib/components/stock/add/LabelBatchQueue.svelte)): each file becomes a row with a thumbnail and a status (queued / reading / ready to review / failed / saved), and files are POSTed to `/api/scan-label` one at a time (concurrency = 1), so the server meters and audits each call on its own. At most 30 files per batch; a file over 10 MB fails its row without being sent. Each successful row carries an `ai`-provenance draft. The operator opens a row ("Review" or "Review next draft") into the **same canonical `A_InventoryEditForm`** used by every other add method, edits it, and saves: one `POST /api/stock` and one audit row per item, after which the flow returns to the queue. There is deliberately **no unattended "Commit all"**: every AI draft is reviewed before it is saved (Invariant 7, AI assists and never gates). Rows have Retry (failed) and Discard actions. A failure that would repeat for every remaining file (no key, monthly spend cap, quota / rate limit, a `fallbackReason` of `no-key | over-cap | rate-limit | offline`, or a network error) **stops the queue**. The operator sees the real reason and how many photos were not sent, plus either the no-key recovery pair (Add Claude key ↗ / Use Manual entry instead →) or a "Resume queue" button. A per-file failure (Claude could not identify the product, timeout, 503 overload) fails only that row, and the queue keeps going. Batch capture from the live camera is still future work.
...
- **Primary path:**
  ...
  4. **Check safety** — kernel evaluates plugin gates (target-pest fit, REI/PHI vs. neighboring activities, environmental thresholds, sprayer chemistry compatibility).
     4a. **Pollinator-protection gate (#130).** The operator answers a required question: "Is the crop or any flowering weed in bloom in this block?" (yes/no). If a crop plugin's `bloomWindow` covers today, the answer is prefilled to _yes_ with provenance `plugin`. It is never prefilled to _no_. The kernel (`lib/safety/pollinatorProtection.ts`) then shows four tiles: bee toxicity · bloom · time of day · residual. Red means block, wheat means warn, forest means pass. Local sunrise and sunset come from a NOAA solar calc (`lib/safety/sunTimes.ts`) at the block's centroid, or the farm lat/lon if the block has none. Blocks: (a) the label prohibits application during bloom and bloom is _yes_ or unanswered; (b) the label is dusk-to-dawn-only, flowers may be present, and the time is between sunrise and sunset; (c) the label is dusk-to-dawn-only and sunrise/sunset can't be computed, until the operator attests that no bees are foraging. A bee-toxic product without label bloom language gets a warning only. A plugin with no `pollinator` data is treated as bloom-prohibited unless its legacy `pollinatorRisk` is `none` or `low`. A block disables **Record application** and cannot be overridden.
...
- **#130 — server enforcement:** `POST /api/insecticide/record` accepts `bloomStatus` (`in-bloom | not-in-bloom | unknown`) and `attestedNoForagers`. If `bloomStatus` is missing (for example, an offline-queue replay from before this change), the server uses `in-bloom` when a crop plugin's bloom window covers the application time, and `unknown` otherwise. It re-runs the gate regardless of UI and returns **422 `POLLINATOR_BLOCK`** with per-check `checks[]` before anything is persisted. A successful response carries `pollinatorWarnings[]`. The gate evaluates against the **application time**, not server receive time: the `/spray/insecticide` page stamps `occurredAt` (epoch ms) when the operator taps Record, and the offline queue stamps any insecticide payload that lacks one at enqueue, so a dusk-to-dawn spray recorded at 21:00 offline and drained at 08:00 is evaluated (and stored) at 21:00. The server rejects an `occurredAt` more than 5 minutes in the future or more than 7 days old (400); the SEASON_CLOSED gate keys off the same time. A missing `occurredAt` falls back to server time. This gate replaces the Phase-25d `pollinatorBloom` dry-run evaluator on the insecticide path; the fungicide path still uses the old one. RULES_VERSION 0.5.6.
- **#130 — attestation on the record:** each persisted `insecticide_events` row stores what the gate evaluated (migration 0046): `bloom_status` (`in-bloom | not-in-bloom | unknown`), `bloom_status_source` (`operator` when the request carried `bloomStatus`, `plugin` when a crop bloom window derived `in-bloom`, `default` when nothing attested it and it fell to `unknown`), `attested_no_foragers` (the request flag, NULL when not sent) and `pollinator_verdict` (the gate's overall `pass | warn` at record time; `block` never persists because the request is refused). Rows recorded before 0046 keep all four NULL and render blank everywhere. Surfaced on the record detail page (`/records/insecticide/[id]` "Pollinator protection" card), the `/records` list detail line, the USDA/NRCS CSV (four columns appended after `record_kind`: `bloom_status`, `bloom_status_source`, `attested_no_foragers` = `yes|no`, `pollinator_verdict`; blank on other kinds), the VDACS audit-pack PDF (a Pollinator column on insecticide rows), and the JSON account export (explicit `null` on legacy rows).
- **#130 — nearby pollinator-attractive blocks (advisory):** the pollinator panel carries a fifth tile, _Nearby pollinator blocks · within 1.0 mi_, listing the Owner's other blocks whose planted crops are in bloom now (crop-plugin bloom window) or are declared bee-attractive (`bloomWindow.beeAttractive: true` on the crop plugin — cucurbits, orchards, small fruit and brambles, clover/vetch/buckwheat/phacelia covers, sunflower, etc.), nearest first with centroid distance (`lib/blocks/distance.ts`) and the reason (`in bloom` or `bee-attractive`). The radius is `NEARBY_POLLINATOR_RADIUS_FT` = 1 mile, the conservative core of the 1–2 mile honey-bee foraging range. A block without mapped geometry is listed as _distance unknown_ and never causes the warning. The tile turns wheat (warn) only when the product is labelled `toxic` or `highly-toxic` and at least one attractive block is in range; otherwise it passes. It never blocks and never disables **Record application**, so it lives outside the safety kernel (`lib/pollinator/nearbyBlocks.ts`, no RULES_VERSION change). Provenance `plugin` (bloom data) + `data` (block geometry). `POST /api/insecticide/record` evaluates the same check from tenant-scoped `listBlocks()` at the application time and appends it to `pollinatorWarnings[]` (id `nearby-blocks`, with `blocks[]` and `unknownDistance[]`) when it warns.
...
## UC-32f — Fungicide spray with FRAC rotation + weather gates

- **Persona:** P1, P2
- **Status:** Implemented at [spray/fungicide/+page.svelte](../apps/web/src/routes/spray/fungicide/+page.svelte) (Phase 21b B-18; weather + FRAC panels Phase 29 / #132). Spec artboard: [`direction-almanac-fungicide.jsx`](design/almanac/direction-almanac-fungicide.jsx).
- **Trigger:** Disease observed while scouting; wet-weather infection risk; calendar "fungicide window"; or ad-hoc.
...
- **Primary path:**
  ...
  2. **Disease + FRAC panel** loads the hourly NWS gridpoint forecast for the block via `GET /api/weather/hourly?blockId=…` (block centroid → first mapped block → saved farm location → Loudoun default; cached 1 h in the global `weather_forecast_cache` under `hourly:<lat>,<lon>`). It renders:
     ...
     - **FRAC rotation tile** — calls the kernel's `checkFracRotation()` client-side against the block's most recent fungicide event (same input the server uses) and shows pass / warn (same FRAC twice in one tank) / block (shares a FRAC group with the last spray) with the groups involved. A block disables Record; the server enforces it regardless (422 `FRAC_ROTATION_BLOCK`).
  3. Optional disease observation; conditions (wind, temp, rain %) and tank size.
  4. Record → `POST /api/fungicide/record` (tank-mix, FRAC rotation, pollinator-bloom, cross-contamination and season-closed gates run server-side). Offline → Dexie queue.
- **Weather is advisory, never a gate:** forecasts are uncertain and the app is offline-first, so no weather rule lives in the safety kernel (`RULES_VERSION` unchanged). When the feed is unreachable, offline, or the block has no location data, every weather value carries a `fallback` provenance badge and the panel says "Weather unavailable — check conditions yourself"; Record is **not** blocked. Forecast-derived values carry `data` provenance; FRAC codes carry `plugin`.
- **Not yet:** disease forecast models (NEWA / FHB) — labelled "coming in Phase 26" with no pass/fail implied. Leaf wetness is an RH proxy, not a sensor or infection model.
- **Success:** Fungicide event recorded with REI/PHI windows; FRAC groups logged for rotation tracking.
...
- **Primary path:**
  ...
  2. **AI** pane (owner-only): paste/clear the Anthropic API key; set the monthly USD cap; set per-endpoint daily call quotas (`suggest`, `succession`, `optimize`, `allocate`, `groups`, `inputs`, `plugin-search`, `plugin-scan`, `scan-label`, `scan-url`, `scan-barcode`). Reaching the cap or a quota never blocks planning: the planning + Plugin Manager endpoints (`/api/plan/{inputs,schedule,allocate}/refine`, `/api/plan/{inputs,schedule,groups,suggest,succession,optimize}`, `/api/plugins/{scan-label,search-by-name,search-by-name/stream,scan-receipt}`, `/api/stock/{short-names,refresh-ai,[id]/refresh-ai}`) route through `aiTry()` via [`aiDegrade.ts`](../apps/web/src/lib/server/aiDegrade.ts) and answer 200 with the deterministic result (or the endpoint's empty shape — `suggestions: []`, local-only search matches, an SSE `error` frame) tagged `provenance: 'fallback'` + `fallbackReason` (`no-key` · `over-cap` · `rate-limit` · `timeout`) + an honest message; the zero-token fallback row does not count toward the quota. A live spend widget shows this farm's month-to-date USD vs cap and a percent-used bar that turns warn-orange at 80%; the per-endpoint tiles show today's token-consuming calls for the signed-in user against each quota (the same count `checkGuard` enforces).
  ...
  4. **Types** pane (owner-only): terms grouped by domain — `inventory:seed`, `inventory:herbicide`, `inventory:insecticide`, …, `equipment`. Add / rename / delete custom terms; system defaults (`isDefault=true`, `owner_id IS NULL`) are shared by every farm, so they are read-only — `PATCH /api/types/:id` on a default returns 400 (Invariant 6; add a custom term instead).
...
- **Token cannot:** mint another token (cookie-session-only surface), switch owners (owner-scoped at issuance; `POST /api/session/switch-owner` returns 403 for `authVia === 'bearer'`). It also cannot exercise superadmin powers, even when the minting user is a superadmin: `requireSuperadmin` returns 403 for `authVia === 'bearer'`, so `/admin/owners` impersonate, exit and billing actions require an interactive cookie session.
- **Cross-tenant isolation:** every endpoint reachable under the Bearer path runs inside `runWithTenantAsync(tokenOwnerId, …)`. Cross-tenant property test extended with a Bearer-authed code path at `apps/web/src/lib/db/tenant.crossTenant.test.ts`. Ids referenced in a request body (`blockId`, `cropId`, `fieldId`, `equipmentId`, `stockItemId`, `typeId`, …) are resolved through the tenant-scoped getters before any write (`rejectForeignRefs` in [`lib/server/foreignRefs.ts`](../apps/web/src/lib/server/foreignRefs.ts)); another Owner's id answers 400 `unknown <field>` on the Bearer, cookie, and offline-queue-drain paths alike.
- **Service-account quota policy:** UI controls per-token `daily_quota_*` columns (defaults 100/100/100/50 — 10× the per-user defaults). The farm's monthly USD cap (measured against that farm's own spend) still applies, and the operator's `AI_GLOBAL_MONTHLY_USD_CAP` is the deployment-wide brake against a runaway agent.
...
- **Audit notes:** New tenant-scoped table `season_closeouts` (migration 0042; CLAUDE.md invariant 6 four-step checklist: `tenantScoped(...)` brand + `owner_id` + composite `(owner_id, year)` UNIQUE index + wired into `tenant.crossTenant.test.ts` + repo funnels through `withTenant`/`tenantValues`). Repo `apps/web/src/lib/db/seasonCloseouts.ts`; gate + close/reopen orchestration `apps/web/src/lib/server/seasonClose.ts`; server preflight `apps/web/src/lib/season/closeout.server.ts`; route `/settings/season/close-out`; endpoint `/api/season/close`. `RULES_VERSION` is **not** bumped — the gate is an app-layer lock (like FR-09), not a safety-kernel rule; the version is stamped into the snapshot for provenance only. Kernel-grade vitest at `apps/web/src/lib/server/seasonClose.test.ts` (fast-check over arbitrary dates vs a closed year + close/reopen/permanent-window state machine + tenant isolation).

---

## UC-45 — Equipment winterization

- **Persona:** P1 (Owner/operator) + P2 (Helper). Helper+ may run the flow — same crew as UC-04 decon; the physical winterization is fieldwork, so a helper can execute it while the API rejects read-only inspectors (`requireMutator`).
- **Status:** Shipped (#350). At season's end the operator puts the sprayer away for winter: a final chemistry-appropriate decon, drain-down, RV-antifreeze flush, nozzle/screen service, and tank inspection — one linear wizard reusing the UC-04 decon chrome (≥48dp CTAs, confirm-to-advance steps).
- **Trigger:** From `/equipment/[id]` → **Winterize** CTA (sprayers only), or from the UC-44 close-out hand-off screen.
- **Primary path:**
  1. `/equipment/[id]/winterize` loads the sprayer and resolves the **class-specific decon SOP** from `equipment_state.last_chemistry_class` via `selectDeconProtocol()`. The wizard walks: final decon → drain-down → RV-antifreeze flush → nozzles/screens → tank inspection → confirm.
  2. On confirm, `POST /api/sprayers/[id]/winterize` writes one `equipment_log` row per step (reusing the existing `decon` / `maintenance` / `inspection` kinds — no enum migration), then in one state write stamps `equipment_state.winterized_at`, clears `last_chemistry_class` + stamps `last_decon_at` (decon semantics), and **nulls calibration** (`calibrated_gpa` / `calibration_date`).
  3. `/equipment/[id]` then reads "Winterized {date} · Recalibrate in spring", and calibration shows "Uncalibrated" — so `/calibrate` engages the Sprint-10 #216 "Uncalibrated" path and UC-10 must re-run before the first spring spray.
- **Class-specific decon SOPs (kernel):** `apps/web/src/lib/safety/deconProtocol.ts` promotes the long-standing "class-specific decon protocols" follow-up to real code. Three strict chemistries replace the generic ammonia soak: **paraquat** (`photosystem-i-diquat`, HRAC 22) → 1% bleach + 1% TSP + 3 water rinses; **glufosinate** (Liberty, HRAC 10) → detergent + water rinse; **copper fungicides** (`fungicide-load`) → vinegar rinse. Every other class — and a clean tank — resolves to the generic ammonia protocol. `RULES_VERSION` bumped 0.5.4 → **0.5.5** (Invariant 1); exhaustive vitest at `deconProtocol.test.ts` (each strict class → correct protocol; default class → generic; every chemistry class swept).
- **Next-spring alert (informational — assists, never gates):** `/today` raises a warning card when a sprayer has current-season activity (spray/decon/calibration) but was not winterized after the prior season (`winterized_at` unset or older than the season start); the season-setup save (`POST /api/season/setup`) returns the same reminder. Pure helper `apps/web/src/lib/today/winterizeAlert.ts` + tests. This is an app-layer nudge, **not** a kernel rule — separate from the RULES_VERSION SOP bump.
- **Audit notes:** New nullable `winterized_at` column on the already-tenant-scoped `equipment_state` (migration `0043_messy_hardball.sql`; no new table — reads/writes still funnel through `withTenant`/`tenantValues`, Invariant 6 holds). Repo helper `recordWinterization()` in `apps/web/src/lib/db/sprayers.ts`.

---

## UC-46 — Year-end summary report

- **Persona:** P1 (P4 receives the PDF, per UC-22). Helpers may view and export (read parity per Invariant 8); there is no mutation surface.
- **Status:** Shipped (#351). A "Year in review" surface on `/records` plus `GET /api/records/year-summary.pdf`, answering "what did this season cost, what did we spray, were we compliant, what did we harvest" on one screen.
- **Trigger:** UC-44 close-out hand-off CTA **Year-end report →**, or the `/records` Year-in-review section (season selector).
- **Preconditions:** Tenant session. A UC-44 close-out is not required — an open season renders the same roll-up.
- **Primary path:**
  1. Operator picks a year on `/records`. `buildYearSummary(year)` pulls rows through the tenant-scoped repos (spray / insecticide / fungicide / harvest events, scout observations, blocks, sprayers, stock movements) and folds them in the pure `computeYearSummary()`.
  2. The roll-up renders: acres treated by product + chemistry class; applications checked against the year's UC-42 philosophy via `isProductAllowed()` (absent setup renders "compliance not evaluated", never a synthesized default); harvest totals by crop + archetype with grain-moisture min/max/mean against the `harvestMoisture.ts` thresholds; input costs pro-rated from lot cost (null-cost lots roll into an "uncosted" bucket rather than $0); the scout→spray funnel; decon + calibration compliance.
  3. **Export PDF** downloads the same aggregate.
- **Deterministic-first (Invariant 7):** the tables are the product; the module never calls Claude and completes end-to-end with no key. The optional AI narrative from the audit spec is **deferred** — if added, it must go through `aiTry()` and render with `Provenance` tagged `ai`.
- **Audit notes:** No migration, no new tables. Pure fold in [yearSummary.ts](../apps/web/src/lib/records/yearSummary.ts) (unit-tested in `yearSummary.test.ts`); orchestration in [yearSummary.server.ts](../apps/web/src/lib/records/yearSummary.server.ts); PDF at [`/api/records/year-summary.pdf`](../apps/web/src/routes/api/records/year-summary.pdf/+server.ts). Every read funnels through `withTenant` (Invariant 6), with a fresh-owner isolation case added to [exports.crossTenant.test.ts](../apps/web/src/lib/db/exports.crossTenant.test.ts).

---

## UC-47 — Full-data carry-forward + rotation advisor

- **Persona:** P1 (Owner/operator). Owner-only — this is next-season planning + stock mutation, gated at the API layer (Invariant 8).
- **Status:** Shipped (#352). Extends the philosophy-only `carryForward()` (UC-42's 6-7 season-setup enums) into whole-operation next-season prep. Deterministic end-to-end (Invariant 7 — "AI assists, never gates"; no AI, no `RULES_VERSION` bump).
- **Trigger:** From the UC-44 close-out hand-off screen (**"Prep next season →"**) or `/settings/season` → **"Prep the {next} season →"**.
- **Primary path:**
  1. `/settings/season/carry-forward` runs a dry-run preview (`carryForwardSeasonAsync({ apply: false })`) and renders five panels + a summary strip. The operator reviews, then **Apply** POSTs `/api/season/carry-forward` with `apply: true`.
  2. **Rotation advisor** — for each block, the prior year's planting families are checked against the plugin/family `agronomy.rotationLookbackYears` plant-back window. Same-family repeats inside the window flag `warn` (≥3-yr plant-back, e.g. solanaceae/brassica) or `suggest` (shorter lookback); outside the window is `ok`.
  3. **Surviving-stock roll-forward** — every non-zero lot is classified `roll` / `flag-expiring` (expires before or within the first 45 days of the new season) / `expired`. On apply, expired lots get a negative `reason='expiry'` movement to zero their balance; the rest roll untouched.
  4. **Planting-template clone** — prior accepted (`harvested`/`active`) plantings clone with dates shifted +1 year, then clamped into the re-validated `scheduleCandidacy` window for the new season, tagged `sourceProvenance='fallback'`. On apply, a carry-forward `wizard_drafts` row (`planId='carry-forward-{year}'`) pre-seeds the AllocationWizard block selection (no new table).
  5. **Calibration-reset hand-off** — sprayers whose calibration is absent (winterization nulls it per UC-45) or stale (older than the prior season start) are flagged for a UC-10 recalibration before the first spring spray.
  6. **Cover-crop N-credit re-key** — the credit is re-keyed onto the ACTUAL terminated `cover-crop.termination` plantings per block (sum of each cover's `defaultCoverCredit`), subsuming #228's real fix. `planInputs()` gains an optional `coverNCreditByBlock` override that wins over the flat season-setup-declared `nCreditForIntent(coverCropIntent)` for blocks the carry-forward supplied.
- **Implementation locus:** pure logic in [`lib/season/carryForwardPlan.ts`](../apps/web/src/lib/season/carryForwardPlan.ts) (no DB, fully unit-tested); server orchestration in [`lib/season/carryForward.server.ts`](../apps/web/src/lib/season/carryForward.server.ts); endpoint [`/api/season/carry-forward`](../apps/web/src/routes/api/season/carry-forward/+server.ts); UI route `/settings/season/carry-forward`; N-credit override in [`lib/plan/inputsPlan.ts`](../apps/web/src/lib/plan/inputsPlan.ts).
- **Deferred:** a dedicated persisted planting-draft table — the cloned plantings (with per-row dates + provenance) are returned in the API response and carried into the wizard in-memory + seed the existing `wizard_drafts` block selection, but full per-planting date/provenance persistence would need its own table; deferred rather than widen the AllocationWizard-bound `wizard_drafts` payload schema.

## UC-48 — Subscription billing (Stripe hosted Checkout + Portal)

- **Persona:** P1 (Owner/operator). Owner-only — helpers/inspectors get 403 at the API layer; a superadmin impersonating an Owner cannot start or change billing.
- **Status:** Shipped (Wave F). Direct Stripe REST (form-encoded, `Bearer STRIPE_SECRET_KEY`, `Idempotency-Key` on every POST) — no SDK. The signed webhook stays the source of truth for plan status.
- **Trigger:** `/settings/billing` → **Start subscription** (no Stripe subscription yet) or **Manage billing** (subscription on file).
- **Primary path:**
  1. Page reads status from `owner_subscriptions` (falls back to `owners.billing_status`) and whether the server has `STRIPE_SECRET_KEY` + `STRIPE_PRICE_ID`.
  2. **Start subscription** → `POST /api/billing/checkout` → `ensureStripeCustomer()` (idempotent; persists `owner_subscriptions.stripe_customer_id`, never overwrites a linked id) → Checkout Session (`mode=subscription`, `client_reference_id=ownerId`, `metadata[owner_id]`, `subscription_data[metadata][ownerId]`) → `{url}` → browser redirects to Stripe.
  3. Stripe returns to `/settings/billing?checkout=success|cancel`; the page shows an honest "status updates once Stripe confirms" note.
  4. Webhook `checkout.session.completed` links customer + subscription to the Owner via `client_reference_id`; `customer.subscription.*` / `invoice.payment_failed` converge status (owner resolved by metadata, then by stored customer id). Status is mirrored onto `owners.billing_status`, so Stripe `unpaid → suspended` engages the suspended-tenant gate and `active` lifts it.
  5. **Manage billing** → `POST /api/billing/portal` → Stripe Billing Portal session (card, invoices, cancel) returning to `/settings/billing`.
- **Alternates:** Billing not configured → endpoints return `503 {error:'billing-not-configured'}` and the page shows "Billing isn't configured on this server" with no action buttons. Stripe API error → `502 {error:'stripe-error'}` + inline retry copy. Suspended Owner → `/settings/billing`, `/api/billing/checkout` and `/api/billing/portal` bypass the billing gate so they can self-remediate. `/api/billing/stripe-webhook` is session-less (signature is the auth).
- **Implementation locus:** [`lib/server/billing/stripeApi.ts`](../apps/web/src/lib/server/billing/stripeApi.ts), [`lib/server/billing/stripe.ts`](../apps/web/src/lib/server/billing/stripe.ts) (webhook), [`/api/billing/checkout`](../apps/web/src/routes/api/billing/checkout/+server.ts), [`/api/billing/portal`](../apps/web/src/routes/api/billing/portal/+server.ts), [`/settings/billing`](../apps/web/src/routes/settings/billing/+page.svelte). No migration (`stripe_customer_id` already existed).

---

## UC-49 — Push alerts (NFR-06)

- **Persona:** P1 (Owner/operator) + P2 (Helper). Any non-inspector member opts in per device, per farm; inspectors are read-only and can't subscribe (`requireMutator`).
- **Status:** Shipped. Web Push with VAPID; no third-party SDK (RFC 8292 VAPID JWT + RFC 8291 `aes128gcm` payload encryption on `node:crypto`, verified byte-for-byte against the RFC 8291 §5 example). Push is **off** unless `VAPID_PUBLIC_KEY` / `VAPID_PRIVATE_KEY` / `VAPID_SUBJECT` are set (`node apps/web/scripts/gen-vapid.mjs`); without them the settings page says "Push isn't configured on this server" and nothing is sent. Alerts are reminders only — the kernel and the FR-09 server lock still enforce regardless.
- **Trigger:** `/settings/notifications` (Settings index → **Notifications**).
- **Primary path:**
  1. The page checks browser support + the production service worker (dev mode doesn't register the SW, so push needs `pnpm build`). **Turn on push alerts** asks for notification permission only on that tap, subscribes via `pushManager.subscribe` with the server's VAPID key (`GET /api/push/vapid-public-key`), and `POST /api/push/subscribe` stores the endpoint + keys under the active Owner and the signed-in user.
  2. Per-kind checkboxes (≥48px rows) choose which alerts this device gets: **Decon due**, **Record lock closing**, **Spring calibration** (`PATCH /api/push/subscribe`). **Send a test notification** (`POST /api/push/test`) pushes only to the caller's own subscriptions. **Turn off on this device** deletes this farm's row (`DELETE /api/push/subscribe`); the browser subscription is kept so other farms on the same device keep working.
  3. An in-process scheduler (started from `hooks.server.ts` `init`; single replica per invariant 3) runs every 15 minutes, visits each Owner with a subscription (skipping suspended/canceled billing), and selects due alerts:
     - **Decon due** — a sprayer still carries chemistry ≥1h after its last spray (and ≤7 days, so a first deploy doesn't page old tanks). All members. Opens `/spray/decon?sprayer=…`.
     - **Record lock closing** — a spray/insecticide/fungicide record will lock under FR-09 within 2 hours. Owners + the record's performer. Opens `/records/{kind}/{id}`.
     - **Spring calibration** — from March on, a sprayer winterized in a prior year (UC-45) still has no calibration. All members, once per sprayer per year. Opens `/calibrate`.
  4. Each alert is claimed in the `push_deliveries` sent-log (UNIQUE `(owner_id, kind, subject_id)`) before sending, so it goes out at most once across ticks and restarts. Recipients must still hold an active non-inspector assignment on that Owner and have the kind enabled.
  5. The service worker's `push` handler shows the notification; `notificationclick` focuses a tab already on the URL, re-uses another app tab, or opens a new one (same-origin paths only).
- **Delivery hygiene:** 404/410 from the push service deletes the subscription; any other failure increments `failure_count` (shown on the settings page); a success resets it.
- **Implementation locus:** tenant-scoped `push_subscriptions` + `push_deliveries` tables (migration 0044; invariant 6 checklist: `tenantScoped(...)` brand + `owner_id` + composite indexes + wired into `tenant.crossTenant.test.ts`), repo [`pushSubscriptions.ts`](../apps/web/src/lib/db/pushSubscriptions.ts); crypto + send [`lib/server/push/webPush.ts`](../apps/web/src/lib/server/push/webPush.ts); pure alert selection [`triggers.ts`](../apps/web/src/lib/server/push/triggers.ts); scheduler [`scheduler.ts`](../apps/web/src/lib/server/push/scheduler.ts) (`PUSH_SCHEDULER=off` disables it; never runs under tests); SW handlers [`lib/client/swPush.ts`](../apps/web/src/lib/client/swPush.ts) compiled to `/sw-push.js` and loaded by the Workbox SW via `importScripts`.
- **Deferred:** email fallback for users without push; signing out doesn't delete the device's subscription rows (the next user who opts in on that device re-points them).

---

## UC-50 — Small-grain plan screen (Zadoks timeline, FHB risk, vernalization)

- **Persona:** P1, P3 (owner/operator); helpers can read it (read-only surface; recording links hidden for the inspector role).
- **Status:** Shipped (#177). Spec artboard: `AWheatPlanScreen` in [`direction-almanac-crops.jsx`](design/almanac/direction-almanac-crops.jsx). Advisory only — no kernel rule, `RULES_VERSION` unchanged.
- **Trigger:** Plan v2 planting card for any planting whose resolved archetype is `small-grain.zadoks` → **"Stages, scab risk & vernalization"**, or directly `/plan/wheat[?planting=<id>]`.
- **Preconditions:** ≥1 small-grain planting in the active Owner's blocks. Weather is optional.
- **Primary path:**
  1. Loader (tenant-scoped `listBlocks` + `listFungicideEvents`) lists every small-grain planting; `?planting=` selects one, otherwise wheat plantings sort first, newest sowing first. A foreign or unknown planting id → 404; a non-small-grain planting → 404. No small-grain plantings → empty state linking to /plan.
  2. **Zadoks timeline** (`lib/plan/smallGrain.ts: buildZadoksTimeline`) — Z00 → Z92 dot track with the current stage highlighted and a key-decision list: herbicide cutoff (Z30 jointing), flag leaf (Z39), heading (Z55), FHB fungicide window (Z61 + ~6 d), harvest (Z92). Growth habit is inferred from the plugin id/name (`winter` / `spring` / `vernal`) or DTM ≥ 180 d. Plugin-authored `growthStageTable` / `zadoksStages` are used (provenance `plugin`), except that winter-habit crops always take their post-dormancy stages (Z30+) from a calendar-anchored typical Mid-Atlantic table (provenance `fallback`, dates shown with "~"), because a days-from-planting offset can't model winter dormancy. Spring-habit crops without a plugin table use typical offsets scaled by DTM.
  3. **FHB (scab) risk proxy** — the client fetches `GET /api/weather/hourly?blockId=…` and counts "scab-favorable" hours (RH ≥ 90% or rain, at 59–86 °F) in the 7 days before projected anthesis (Z61), scaled to a full 168-h window: < 24 h low, 24–48 moderate, ≥ 48 high. Shows "Not in window" when that window is beyond the forecast, "Window closed" after Z61 + 6 d, "Unknown / weather unavailable" on the fallback path. A daily scab-favorable-hours SVG bar chart shades the pre-flowering days. Fungicide records on the block within ±21 d of flowering are listed (provenance `data`). Copy links to wheatscab.psu.edu and says the model is a proxy.
  4. **Vernalization** — winter habit only: each hour since sowing at 32–50 °F counts 1/24 cold day toward a 42-day requirement. Hours in the forecast feed use its temperatures (`data`); earlier hours use a Dulles (IAD) 1991–2020 normals climatology with a ±9 °F diurnal swing (`fallback`); forecast hours after now show as a projected extension; accumulation stops May 1 of the spring after sowing. Spring-sown winter varieties and incomplete progress after Apr 15 are flagged "At risk". Spring habit → "Not required".
- **Offline / no weather:** timeline and vernalization (climatology) still render; FHB shows the weather-unavailable state. Nothing blocks.
- **Implementation locus:** [`lib/plan/smallGrain.ts`](../apps/web/src/lib/plan/smallGrain.ts) (pure, client-safe), [`routes/plan/wheat/`](../apps/web/src/routes/plan/wheat/), `ZadoksTimeline` / `FhbRiskPanel` / `VernalizationPanel` in `lib/components/plan/`. The harvest-side bushels/moisture/test-weight form stays in the `SmallGrainZadoks` harvest renderer (#144).
- **Deferred:** a calibrated FHB model (the national scab-initiative model isn't a public API); observed weather history (only the forecast feed's hours are used); planting-level `archetypeOverride` is not consulted (the blocks repo doesn't expose it).
...
| UC-17  | Implemented                                                                  | All        | [+page.svelte](../apps/web/src/routes/+page.svelte) + [lib/server/magicLink.ts](../apps/web/src/lib/server/magicLink.ts)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| UC-20  | Implemented                                                                  | P5         | [/onboarding/+page.svelte](../apps/web/src/routes/onboarding/+page.svelte) (Phase 18f; two screens + Getting Started in Phase 30B)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| UC-32f | Implemented (#132)                                                           | P1, P2     | [spray/fungicide/+page.svelte](../apps/web/src/routes/spray/fungicide/+page.svelte) + hourly NWS feed [weatherHourly.ts](../apps/web/src/lib/server/weatherHourly.ts) + pure [leafWet.ts](../apps/web/src/lib/weather/leafWet.ts) + `GET /api/weather/hourly`; weather advisory, FRAC rotation server-enforced                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| UC-44  | **Shipped** (#349)                                                           | P1         | `season_closeouts` table (migration 0042) + [seasonCloseouts.ts](../apps/web/src/lib/db/seasonCloseouts.ts) repo + shared `SEASON_CLOSED` gate in [seasonClose.ts](../apps/web/src/lib/server/seasonClose.ts) (called from spray/insecticide/fungicide/harvest/hay-cuttings record endpoints) + server preflight [closeout.server.ts](../apps/web/src/lib/season/closeout.server.ts) + `/settings/season/close-out` route + `/api/season/close` endpoint (close + 7-day reversible reopen).                                                                                                                                                                                                                                  |
| UC-45  | **Shipped** (#350)                                                           | P1, P2     | Nullable `winterized_at` on `equipment_state` (migration 0043) + `recordWinterization()` in [sprayers.ts](../apps/web/src/lib/db/sprayers.ts) + `/equipment/[id]/winterize` route (reuses UC-04 decon chrome) + `/api/sprayers/[id]/winterize` (helper+ via `requireMutator`) + class-specific decon SOP kernel [deconProtocol.ts](../apps/web/src/lib/safety/deconProtocol.ts) (RULES_VERSION → 0.5.5) + spring reminder [winterizeAlert.ts](../apps/web/src/lib/today/winterizeAlert.ts) on `/today` + season-setup save. Winterization nulls calibration → UC-10 re-runs in spring.                                                                                                                                       |
| UC-46  | **Shipped** (#351)                                                           | P1, P4     | Pure fold [yearSummary.ts](../apps/web/src/lib/records/yearSummary.ts) + orchestration [yearSummary.server.ts](../apps/web/src/lib/records/yearSummary.server.ts) + Year-in-review section on [records/+page.svelte](../apps/web/src/routes/records/+page.svelte) + [`/api/records/year-summary.pdf`](../apps/web/src/routes/api/records/year-summary.pdf/+server.ts). Deterministic, no migration; AI narrative deferred.                                                                                                                                                                                                                                                                                                   |
| UC-47  | **Shipped** (#352)                                                           | P1         | Pure logic [carryForwardPlan.ts](../apps/web/src/lib/season/carryForwardPlan.ts) (rotation advisor + stock roll-forward + planting clone + calibration hand-off + cover-crop N-credit re-key) + server orchestration [carryForward.server.ts](../apps/web/src/lib/season/carryForward.server.ts) + [`/api/season/carry-forward`](../apps/web/src/routes/api/season/carry-forward/+server.ts) + `/settings/season/carry-forward` UI (entry from UC-44 close-out + /settings/season). N-credit re-key onto actual terminated covers via `coverNCreditByBlock` in [inputsPlan.ts](../apps/web/src/lib/plan/inputsPlan.ts) (subsumes #228). Reuses `wizard_drafts` — no migration. Deterministic (no AI, no RULES_VERSION bump). |
| UC-48  | **Shipped** (Wave F)                                                         | P1         | [stripeApi.ts](../apps/web/src/lib/server/billing/stripeApi.ts) (customer + Checkout + Portal via direct REST) + webhook [stripe.ts](../apps/web/src/lib/server/billing/stripe.ts) + `/api/billing/{checkout,portal}` + `/settings/billing`. Env: `STRIPE_SECRET_KEY`, `STRIPE_PRICE_ID`, `STRIPE_WEBHOOK_SECRET`. No migration.                                                                                                                                                                                                                                                                                                                                                                                             |
| UC-49  | **Shipped** (NFR-06)                                                         | P1, P2     | `push_subscriptions` + `push_deliveries` tables (migration 0045) + [pushSubscriptions.ts](../apps/web/src/lib/db/pushSubscriptions.ts) + [lib/server/push/](../apps/web/src/lib/server/push/) (VAPID + RFC 8291 encryption, trigger selection, 15-min in-process scheduler) + `/api/push/*` + `/settings/notifications` + SW handlers in [swPush.ts](../apps/web/src/lib/client/swPush.ts)                                                                                                                                                                                                                                                                                                                                   |
| UC-50  | **Shipped** (#177)                                                           | P1, P3     | Pure model [smallGrain.ts](../apps/web/src/lib/plan/smallGrain.ts) + [/plan/wheat](../apps/web/src/routes/plan/wheat/+page.svelte) + ZadoksTimeline / FhbRiskPanel / VernalizationPanel                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
