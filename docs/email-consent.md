...
- Consent is per (farm, user, alert kind) in the tenant-scoped `email_alert_consents` table (migration 0057). No row means off. Opting in stores the time, the source (`settings` or `unsubscribe-page`) and the client IP; opting out keeps the row with `status = 'opted-out'`.
