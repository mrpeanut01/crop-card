import { expect, test, settleForScreenshot } from '../lib/test';
 ...
 * the diff before committing the new PNGs.
 *
  ...
  for (const vp of VIEWPORTS) {
    test(`${route.name} spray page at ${vp.name} (${vp.width}x${vp.height})`, async ({ page }) => {
      ...
      await expect(page).toHaveScreenshot(`spray-${route.name}-${vp.name}.png`, {
        ...
        animations: 'disabled',
        mask: [
          page.locator('[data-test="now"], [data-live="weather"], time'),
          page.locator('small', { hasText: /^id: / })
        ]
      });
    });
  }
